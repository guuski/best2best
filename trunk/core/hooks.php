  ...
  function bp_reviews_format_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	switch ( $action ) {
		case 'new_review':
			...
			$review_title = __('New reviews','reviews');

			if ( (int)$total_items > 1 ) {
				$text_title = sprintf( __( 'You have recieved %1$d new reviews', 'reviews' ), (int)$total_items );
				...
				$text_title =  sprintf( __( '%1$s wrote you a new review', 'reviews' ), $user_fullname );
		...
		break;
			...
			$review_title = __('Approved reviews','reviews');

			if ( (int)$total_items > 1 ) {
				$text_title = sprintf( __( 'Congrats! %1$d of your reviews were approved', 'reviews' ), (int)$total_items );
				...
				$text_title =  sprintf( __( '%1$s approved Your review', 'reviews' ), $user_fullname );
...
function bp_review_build_notification($notifications){
    ...
    foreach($notifications as $notification){
       ...
       if($notification->component_action=='new_review'){
           $review_link  = bp_loggedin_user_domain() . $bp->reviews->slug . '/pending/';
           $user_fullname = bp_core_get_user_displayname( $poster_user_id );
	   $text =  sprintf( __( '%1$s wrote you a new review', 'reviews' ), $user_fullname );
	...
	$text =  sprintf( __( '%1$s approved your  review', 'reviews' ), $user_fullname );
	...
	<table class="notification-settings" id="activity-notification-settings">
		...
		<tbody>
			<tr id="review-notification-settings-new-review">
				<td>&nbsp;</td>
				<td><?php _e( 'A member writes you a review', 'reviews' ) ?></td>
			...
			<tr id="review-notification-settings-approved-review">
				<td>&nbsp;</td>
				<td><?php _e( "When a user approves your review", 'reviews' ) ?></td>
