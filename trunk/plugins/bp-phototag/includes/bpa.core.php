...
function bp_album_add_admin_menu() {
	...
	add_menu_page(__( 'BP Photos+tags', 'bp-phototag' ), __( 'BP Photos+tags', 'bp-phototag' ), 'administrator', 'bp-phototag-settings', 'bp_album_admin' );
...
function bp_album_setup_nav() {
	...
	$nav_item_name = apply_filters( 'bp_album_nav_item_name', __( 'Album', 'bp-phototag' ) );
	...
	$album_link_title = ($bp->displayed_user->id) ? bp_word_or_name( __( "My pictures", 'bp-phototag' ), __( "%s's pictures", 'bp-phototag' ) ,false,false) : __( "My pictures", 'bp-phototag' );
	...
	if($bp->current_component == $bp->album->slug  && $bp->current_action == $bp->album->single_slug ){
		...
		bp_core_new_subnav_item( array(
			'name' => isset($pictures_template->pictures[0]->id) ? bp_album_get_picture_title_truncate(20) :  __( 'Picture', 'bp-phototag' ),
	...
	bp_core_new_subnav_item( array(
		'name' => __( 'Upload picture', 'bp-phototag' ),
...
function bp_album_record_activity($pic_data) {
	...
	$action = sprintf( __( '%s uploaded a new picture: %s', 'bp-phototag' ), bp_core_get_userlink($pic_data->owner_id), '<a href="'. $primary_link .'">'.$title.'</a>' );
...
function init_photo_tagging() {
    ...
    if ($bp->current_component == 'album') {
      ...
      //  wp_deregister_script( 'jquery' );
        ...
        wp_register_script( 'jqueryui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.7/jquery-ui.min.js');
