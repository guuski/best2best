...
function bp_dtheme_ajax_messages_autocomplete_results() {
	...
	//error_log("autoc1omplete: ".$autocomplete_all. "  ".$bp->messages->autocomplete_all."-".$bp->messages->slug."-".$bp->current_component);
	
	$pag_page = 1;
	$limit    = !empty( $_GET['limit'] ) ? $_GET['limit'] : apply_filters( 'bp_autocomplete_max_results', 10 );

	// Get the user ids based on the search terms
	if ( !empty( $autocomplete_all ) || true ) {
	...
	if ( !empty( $user_ids ) ) {
		foreach ( $user_ids as $user_id ) {
			...
			echo '<span class="gbp" id="link-' . $username . '" href="' . bp_core_get_user_domain( $user_id ) . '"></span>' . bp_core_fetch_avatar( array( 'item_id' => $user_id, 'type' => 'thumb', 'width' => 15, 'height' => 15 ) ) . ' &nbsp;' . bp_core_get_user_displayname( $user_id ) . ' (' . $username . ')
			';
		}
	}
}
