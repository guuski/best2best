...
{
    var $object_id; //userid/group id etc
    var $post_id; //the post mapped for this objects review
        
	//get the id of post associated with this user as Review page 	
	
	abstract function get_post_id();
	abstract function get_post_type(); 						//what post type is used to store this review						 
	abstract function associate_review_page($object_id='');	
	abstract function get_component_link(); 				//returns the link to current object's review page 
															//eg. http://example.com/members/reviews
	function get_post()
	{
	...
	function get_comment_by_status($is_approved,$page=1,$limit=20)
	{
		...
		$or_sql='';
		//l utente loggato può vedere le sue reviews anche se sono pendenti
   ...
   function attach(SplObserver $observer) 
   {
	...
	function save_review($content)
	{
		...
		$review_data= 
			array(	'comment_content' => apply_filters( 'bp_review_content', $content ),
					'comment_approved'=>0,
...
class BPUserReviewMapper extends BPReviewMapper
{
    ...
    function associate_review_page($user_id=null)
	{
       ...
       $post_id=$this->get_post_id();
		...
		if(empty($post_id))
		{         
			$title=sprintf(__("Review Page for User [%s] with ID:[%d]",'reviews'),bp_core_get_user_displayname($user_id),$user_id);
			$postarray=array('post_title'=>$title,'post_author'=>$user_id,'post_type'=>'ureviews','post_status'=>'publish');
	...
	function notify_by_email($user_id,$other_id,$comment_id,$action)
	{
		...
		switch($action){
			case 'new_review':
				...
				$subject=sprintf(__('%s wrote you a new review','reviews'),$user_name);                      
				$message = sprintf( __('%1$s wrote you a new review:"%2$s"To view it, please log in and visit: %3$s
				---------------------', 'reviews' ), $user_name, $content, $review_link );		                
				break;
				break;
			case 'approved_review':
				$review_link  = bp_core_get_user_domain($other_id) . $bp->reviews->slug ;
				$subject=sprintf(__('%s approved your review','reviews'),$user_name);                    
				$message = sprintf( __('%1$s accepted your review:"%2$s"To view it, please log in and visit: %3$s
				---------------------', 'reviews' ), $user_name, $content, $review_link );					
			break;
		}
