...
class BPUserReview extends BP_Component{
	...
	function setup_globals() {
        ...
        function setup_nav() {
            ...
            if(bp_is_my_profile()){
                $nav_text=sprintf(__('My Reviews <span>%d</span>','reviews'),$this->mapper->get_comment_count_by_status(1));
                $review_link = trailingslashit( $bp->loggedin_user->domain . $this->slug );//without a trailing slash
            }
            else{
                $nav_text=sprintf (__('%s\'s Reviews <span>%d</span>', 'reviews'),  bp_core_get_user_displayname ($bp->displayed_user->id),$this->mapper->get_comment_count_by_status(1));
            ...
            $sub_nav[] = array(
                    'name'            => sprintf(__('Pending <span>%d</span>', 'reviews' ),$this->mapper->get_comment_count_by_status(0)),
                    'slug'            => 'pending',
                    'parent_url'      => $review_link,
                    'parent_slug'     => $this->slug,
	 ...
	 parent::setup_nav( $main_nav, $sub_nav );//will call bp_ureview_setup_nav
      ...
      /*action functions*/
      //a little bit repitation of code here from inc/ajax.php, still thinking on how to avoid it in future  
	...
	if ( !is_user_logged_in() ) {
		...
		$message= __( 'Please enter something text.', 'reviews' ) ;
		$error=true;
	}
        else if(!BPReviewUserPermissions::can_write()){
              $message=__('You are not allowed to write a review.','reviews');
              $error=true;
        }
        else{
            ...
            $message=__('Your review is posted. It will appear publicly once approved.','reviews');
            if ( empty( $review_id ) ) {
                $message= __( 'There was a problem posting your review, please try again.', 'reviews' ) ;
                $error=true;
            }
            else
	...
	if ( !is_user_logged_in() ) {
           ...
           $error=true;
            ...
            $message= __( 'Your action is not valid!.', 'reviews' );
            $error=true;
        ...
        if(!$error){
           ...
           $review=BPReviewMapper::get_comment($review_id);
            ...
            if(!BPReviewUserPermissions::can_approve($review,$this->mapper)){
                  $message=__('You don\'t have the rights to performs this action.','reviews');
                  $error=true;
            }
            ...
            if(!$error&&$this->mapper->update_review($review)){
                ...
                $message= __( 'There was a problem performing the action!. Please try again later!', 'reviews' );
                $error=true;
          }
     }  
     bp_core_add_message($message,$error);
